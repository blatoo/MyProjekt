...
public class ExperimentalTesterForNetflixData {

	private static boolean debug = false;
	private static boolean notitle = false;
	private static boolean plot = true;
	private static int numberOfFilms = 17770;
	private static int numberOfUserPerMatrix = 20000;


	public ExperimentalTesterForNetflixData() {
	...
	public static void main(String[] args){
		...
		runAnalysis(numberOfUserPerMatrix, numberOfSamples, lengthOfWalk, "/home/nina/Data/Netflix/NewCoocc", startingTheTesting, numberOfTests);
	}
				...
				while(line != null){
					...
					if(counter == numberOfUsersPerMatrix){
						if(dataSetCounter > startingTheTesting){
							...
							printCoocc(coocc, outputFile, dataSetCounter, numberOfSamples, degreeOfFilms);
	...
	private static void printCoocc(int[][] coocc, String outputDir,
			int numberOfDataSet, int numberOfSamples, short[] degreesOfFilms) {
...
//			for(int i = 0; i < 1000; ++i){
				if(degreesOfFilms[i] != 0){
					...
					for(int j = 0; j < i; ++j){
						if(coocc[i][j] != 0)
							bw.write(i+" "+j+" "+((double)coocc[i][j]/(double)numberOfSamples)+"\n");
					}
				}
			}
			bw.close();
